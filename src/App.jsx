 ...
 * ✅ Firebase Auth untuk admin (hapus PIN hardcode)
 * ✅ genId pakai crypto.randomUUID() — tidak ada collision
 * ✅ Hapus Easter egg tap logo 7x
 * ✅ Nomor WA & alamat diisi dengan placeholder — ganti sesuai data asli
 * ✅ Admin hanya bisa diakses dari menu Profile (setelah login)
 * ✅ myBookings tetap di localStorage (customer tidak perlu login)
 */

import { useState, useCallback, useEffect, useRef } from "react";
import {
  Home, Calendar, Tag, MapPin, User, Search,
  Sun, Moon, Smartphone, Wrench, ChevronRight,
  ...
  X, Bell, Info, Lock, Settings, Trash2, LogOut,
} from "lucide-react";
...
const SHOP = {
  ...
  lat:       -7.5888545,
  lng:       110.796342,
  mapsEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d991.4765826316235!2d110.79634218561856!3d-7.588854499999997!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2e7a166d3374f6a3%3A0x2ab32c2e2eb93bdd!2sDaniel%27s+Repair+Center!5e0!3m2!1sid!2sid!4v1714900000000!5m2!1sid!2sid",
};
...
// ✅ genId menggunakan crypto.randomUUID() — dijamin unik
const genId  = () => {
...
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  :root {
    --gold:#D4A017; --gold-light:#F0B429; --gold-dim:rgba(212,160,23,0.12); --gold-border:rgba(212,160,23,0.25);
    --bg:#09090f; --bg2:#111118; --bg3:#18181f; --bg4:#1e1e28;
    --text:#f0f0f0; --text2:#9a9aaa; --text3:#4a4a5a;
    --border:#1e1e28; --border2:#252535;
    --blue:#60a5fa; --blue-dim:rgba(96,165,250,0.12); --blue-border:rgba(96,165,250,0.25);
    --green:#34d399; --green-dim:rgba(52,211,153,0.12); --green-border:rgba(52,211,153,0.25);
    --red:#f87171; --red-dim:rgba(248,113,113,0.12); --red-border:rgba(248,113,113,0.25);
    --amber-dim:rgba(212,160,23,0.12); --amber-border:rgba(212,160,23,0.3);
  }
  [data-theme="light"] {
    --bg:#f5f5f0; --bg2:#ffffff; --bg3:#f0f0ea; --bg4:#e8e8e0;
    --text:#111111; --text2:#555566; --text3:#aaaaaa;
    --border:#e2e2d8; --border2:#d5d5cc;
    --blue:#2563eb; --blue-dim:rgba(37,99,235,0.08); --blue-border:rgba(37,99,235,0.2);
    --green:#059669; --green-dim:rgba(5,150,105,0.08); --green-border:rgba(5,150,105,0.2);
    --red:#dc2626; --red-dim:rgba(220,38,38,0.08); --red-border:rgba(220,38,38,0.2);
    --amber-dim:rgba(212,160,23,0.1); --amber-border:rgba(212,160,23,0.3);
  }
  ...
  .topbar { position:sticky; top:0; z-index:100; background:var(--bg); border-bottom:1px solid var(--border); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); transition:background .25s,border-color .25s; }
  ...
  .logo-box { width:36px; height:36px; border-radius:10px; background:var(--gold-dim); border:1px solid var(--gold-border); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
  ...
  .bottom-nav { position:fixed; bottom:0; left:0; right:0; z-index:100; background:var(--bg); border-top:1px solid var(--border); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); transition:background .25s,border-color .25s; }
  ...
  @keyframes glow-text { 0%{text-shadow:0 0 5px rgba(212,160,23,0.1)} 50%{text-shadow:0 0 15px rgba(212,160,23,0.4)} 100%{text-shadow:0 0 5px rgba(212,160,23,0.1)} }
  .animate-glow { animation:glow-text 3s infinite ease-in-out; }
    ...
    <div className="gap-4">
      <div className="card-gold card-p" style={{ padding:20 }}>
        ...
        <p style={{ fontSize:13, color:"var(--text2)", margin:"0 0 16px", lineHeight:1.6 }}>
          Cepat, bergaransi, harga transparan. Booking online — tidak perlu antre panjang!
        </p>
      ...
      <div className="stats-grid">
        {[
          { v:"2.400+", l:"Pelanggan Puas",   I:ThumbsUp },
          { v:"5+ Tahun", l:"Pengalaman",      I:Award    },
          { v:"30 Hari",  l:"Garansi Servis",  I:Shield   },
          { v:"4.9 ★",    l:"Rating Google",   I:Star     },
        ].map(({ v,l,I }) => (
      ...
      <div className="wa-banner">
        ...
        <a className="wa-btn" href={`https://wa.me/${SHOP.phone}`} target="_blank" rel="noopener noreferrer">Chat</a>
      </div>
    </div>
  );
}
...
// ─── PAGE BOOKING ─────────────────────────────────────────────────────────────
function PageBooking({ addToast, setMyBookings }) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setStep(2);
      addToast("success","Booking Berhasil! 🎉", `ID: ${id} · Hadir 5 menit sebelum jadwal`);
    } catch (err) {
      console.error(err);
      addToast("error","Gagal Booking","Periksa koneksi internet dan coba lagi");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (step === 1) return (
    <div className="gap-4">
      ...
      <button className="btn-gold" style={{ width:"100%", height:48 }} onClick={confirm} disabled={loading}>
        {loading ? "Menyimpan..." : <><CheckCircle size={15}/> Konfirmasi Booking</>}
      </button>
    </div>
  );
  ...
  if (step === 2 && result) return (
    <div className="gap-4">
      ...
      <div className="banner-info"><Info size={14}/><span className="banner-text">Simpan Booking ID ini untuk lacak status perbaikan kapan saja.</span></div>
...
function PageTrack({ queue, addToast }) {
  ...
  const search = async () => {
    ...
    setSearching(true);
    try {
      const qry  = query(collection(db, "bookings"), where("id", "==", val), limit(1));
      const snap = await getDocs(qry);
      if (!snap.empty) {
        setResult({ ...snap.docs[0].data(), docId:snap.docs[0].id });
        setNotFound(false);
        addToast("info","Booking ditemukan",`Status: ${snap.docs[0].data().status}`);
      } else {
        setResult(null);
        setNotFound(true);
  ...
  // realtime sync — jika result sudah ditemukan, ikuti update dari queue
    ...
    <div className="gap-4">
      ...
      <div className="card card-p">
        ...
        <div style={{ display:"flex", gap:8 }}>
          <input className="inp font-mono" style={{ flex:1, textTransform:"uppercase" }} placeholder="Contoh: DRC-A1B2C3"
            value={input} onChange={e => setInput(e.target.value.toUpperCase())} onKeyDown={e => e.key==="Enter" && search()}/>
          <button className="btn-gold" style={{ height:46, padding:"0 18px", flexShrink:0 }} onClick={search} disabled={searching}>
            <Search size={15}/> {searching ? "..." : "Cari"}
          </button>
        </div>
      </div>
      {notFound && !live && (
        <div className="banner-red"><AlertCircle size={14}/><div>
          <div style={{ fontSize:13, fontWeight:600, color:"var(--red)" }}>Booking tidak ditemukan</div>
          <div className="banner-text" style={{ marginTop:3 }}>Pastikan ID booking sudah benar. ID tersedia di tiket booking kamu.</div>
        </div></div>
      )}
      {live && (
        <div className="gap-4">
          ...
          {live.status === "Selesai" && (
            <div className="banner-green"><CheckCircle size={14}/><div>
              <div style={{ fontWeight:600, color:"var(--green)", fontSize:13 }}>Perangkat Siap Diambil!</div>
              <div className="banner-text" style={{ marginTop:3 }}>Datang ke toko dan lakukan pembayaran di kasir.</div>
            </div></div>
          )}
        </div>
      )}
    ...
    <div className="map-wrap">
      <svg width="100%" height="280" viewBox="0 0 400 280" preserveAspectRatio="xMidYMid slice" style={{ position:"absolute", inset:0, display:"block" }}>
        ...
        <g fill={dark?"#1e2436":"#cddfc2"}>
          ...
          <rect x="10" y="170" width="50" height="35" rx="2"/><rect x="70" y="175" width="65" height="28" rx="2"/>
      ...
      <iframe title="Lokasi" src={SHOP.mapsEmbed} className="map-iframe"
  ...
  return (
    <div className="gap-4">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        ...
        <a href={`https://wa.me/${SHOP.phone}?text=Halo%2C%20saya%20mau%20tanya%20lokasi%20tokonya`} target="_blank" rel="noopener noreferrer"
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {[
          { I:MapPin,    l:"Alamat",       v:SHOP.address    },
          { I:Clock,     l:"Jam Buka",     v:SHOP.hours      },
          { I:Phone,     l:"Telepon / WA", v:SHOP.phoneDisplay },
          { I:Instagram, l:"Instagram",    v:SHOP.instagram  },
        ].map(({ I,l,v }) => (
...
// ─── PAGE PROFILE ─────────────────────────────────────────────────────────────
function PageProfile({ setPage, dark, toggleDark, queue, myBookings, addToast, adminUser }) {
  ...
  const menuItems = [
    ...
    { I:MessageCircle, l:"WhatsApp CS",    s:"Konsultasi teknisi langsung",  action:() => window.open(`https://wa.me/${SHOP.phone}`, "_blank") },
    { I:Info,          l:"Tentang DRC",    s:"v1.1.0 — iPhone Specialist",   action:() => addToast("info","Daniel's Repair Center","Spesialis iPhone Wonogiri. Garansi 30 hari.") },
  ];

  // Hanya tampilkan menu Admin kalau sudah login
  if (adminUser) {
    menuItems.push({ I:Settings, l:"Admin Dashboard", s:"Kelola antrean & status", action:() => setPage("admin") });
  }
    ...
    <div className="gap-4">
      <div className="card-gold card-p" style={{ textAlign:"center", padding:20 }}>
        ...
        <div style={{ fontSize:17, fontWeight:800, color:"var(--text)", marginBottom:4 }}>
          {adminUser ? "Admin — Daniel's RC" : "Halo, Pelanggan!"}
        </div>
        <div style={{ fontSize:12, color:"var(--text2)", marginBottom:12 }}>
          {adminUser ? adminUser.email : "Pusat Layanan Daniel's Repair Center"}
        </div>
        <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"var(--gold-dim)", border:"1px solid var(--gold-border)", borderRadius:100, padding:"5px 14px", fontSize:11, fontWeight:700, color:"var(--gold)" }}>
          <Shield size={10} style={{ fill:"var(--gold)" }}/> {adminUser ? "Admin Account" : "Guest Account"}
        </div>
      ...
      <div className="card card-p-sm" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer" }} onClick={toggleDark}>
      ...
      <div style={{ textAlign:"center", fontSize:11, color:"var(--text3)", paddingBottom:8 }}>
        Daniel's Repair Center · iPhone Specialist · Wonogiri<br/>v1.1.0
      </div>
    </div>
  );
}

// ─── PAGE ADMIN — Firebase Auth ───────────────────────────────────────────────
function PageAdmin({ queue, addToast, adminUser, onLogout }) {
  const [email,setEmail]       = useState("");
  const [password,setPassword] = useState("");
  const [loginLoading,setLoginLoading] = useState(false);

  const handleLogin = async () => {
    if (!email || !password) return addToast("warn","Isi email & password","");
    setLoginLoading(true);
    try {
      await signInWithEmailAndPassword(auth, email, password);
      addToast("success","Login berhasil","Selamat datang, Admin!");
    } catch {
  ...
  const advance = async (docId, currentStatus) => {
    const next = { Menunggu:"Dikerjakan", Dikerjakan:"Selesai", Selesai:"Selesai" }[currentStatus];
    try {
      await updateDoc(doc(db, "bookings", docId), { status:next });
      addToast("info","Status diperbarui",`→ ${next}`);
    } catch {
      addToast("error","Gagal","Coba lagi");
    }
  };
  ...
  const deleteBooking = async (docId) => {
    ...
    try {
      ...
      addToast("success","Dihapus","Booking telah dihapus");
    } catch {
      addToast("error","Gagal hapus","Coba lagi");
    }
  };
  ...
  if (!adminUser) return (
    <div className="admin-lock">
      ...
      <h2 style={{ fontSize:20, fontWeight:800, color:"var(--text)", margin:"0 0 6px" }}>Admin Login</h2>
      <p style={{ fontSize:13, color:"var(--text2)", margin:"0 0 20px" }}>Masukkan email & password admin</p>
      <div style={{ width:"100%", maxWidth:300, display:"flex", flexDirection:"column", gap:10 }}>
        <input className="inp" type="email" placeholder="Email admin"
          value={email} onChange={e => setEmail(e.target.value)}/>
        <input className="inp" type="password" placeholder="Password"
          value={password} onChange={e => setPassword(e.target.value)}
          onKeyDown={e => e.key==="Enter" && handleLogin()}/>
        <button className="btn-gold" style={{ width:"100%", height:46 }} onClick={handleLogin} disabled={loginLoading}>
  ...
  const stats = {
    Total:   queue.length,
    Antre:   queue.filter(q => q.status==="Menunggu").length,
    Proses:  queue.filter(q => q.status==="Dikerjakan").length,
    Selesai: queue.filter(q => q.status==="Selesai").length,
  };
    ...
    <div className="gap-4">
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <button onClick={onLogout}
          style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 12px", borderRadius:8, fontSize:12, fontWeight:600, border:"1px solid var(--red-border)", color:"var(--red)", background:"var(--red-dim)", cursor:"pointer" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 }}>
        {Object.entries(stats).map(([l,v]) => (
          <div key={l} className="card" style={{ padding:"10px 6px", textAlign:"center" }}>
            <div style={{ fontSize:20, fontWeight:800, color:l==="Proses"?"var(--blue)":l==="Selesai"?"var(--green)":"var(--gold)" }}>{v}</div>
            <div style={{ fontSize:10, color:"var(--text3)", marginTop:2 }}>{l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {queue.length === 0 && (
          <div style={{ textAlign:"center", padding:"40px 0", fontSize:13, color:"var(--text3)" }}>Belum ada antrean</div>
        )}
        {queue.map(item => (
          <div key={item.docId} className="card card-p" style={{ opacity:item.status==="Selesai"?.5:1 }}>
...
export default function App() {
  ...
  // ✅ Firebase Auth state — satu sumber kebenaran untuk admin
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(qry, snap => {
      setQueue(snap.docs.map(d => ({ ...d.data(), docId:d.id })));
      setLoading(false);
    });
    return () => unsub();
  }, []);

  // localStorage untuk riwayat booking customer (tidak perlu login)
  const [myBookings,setMyBookings] = useState(() => {
    try { return JSON.parse(localStorage.getItem("drc_user_bookings") || "[]"); }
    catch { return []; }
  });
  useEffect(() => {
  ...
  const handleLogout = async () => {
    await signOut(auth);
    setPage("home");
    addToast("info","Logout","Sesi admin berakhir");
  };
        ...
        <div className="page">
          {!isReady ? (
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"60vh", gap:12 }}>
              ...
              <div style={{ fontSize:14, fontWeight:600, color:"var(--text2)" }}>Memuat...</div>
            </div>
          ) : (
            <>
              {page==="home"      && <PageHome      setPage={setPage}/>}
              {page==="booking"   && <PageBooking   addToast={addToast} setMyBookings={setMyBookings}/>}
              {page==="pricelist" && <PagePricelist/>}
              {page==="track"     && <PageTrack     queue={queue} addToast={addToast}/>}
              {page==="maps"      && <PageMaps      dark={dark}/>}
              {page==="profile"   && <PageProfile   setPage={setPage} dark={dark} toggleDark={() => setDark(d=>!d)} queue={queue} myBookings={myBookings} addToast={addToast} adminUser={adminUser}/>}
              {page==="admin"     && <PageAdmin     queue={queue} addToast={addToast} adminUser={adminUser} onLogout={handleLogout}/>}
            </>
          )}
        </div>

        {/* BOTTOM NAV — sembunyikan halaman admin dari nav */}
